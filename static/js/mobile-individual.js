...
var indimain = function() {
	...
	function getData() {
		...
		function spit(resp){
			...
			$('#short').append(currently.caption);
				...
				if ( !(cursor == null || cursor == '') ) {
					...
					switch (cursor.type) {
						...
						case 'image':
							var image = '';
							if (cursor.file != null){
								image += '<img style="max-height: 100%;max-width: 100%;" src="' + cursor.file + '">';
							} else {
								image += '<img style="max-height: 100%;max-width: 100%;" src="' + cursor.link + '">';
							}
							total += image;
							break;
						default:
